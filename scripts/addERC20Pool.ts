...
async function main() {
  ...
  const initialSupply = ethers.parseEther("1000000"); // 100 万枚
  ...
  try {
    console.log("正在发送 addPool 交易（ERC20 池子）...");

    const tx = await MetaNodeStake.connect(signer).addPool(
      testTokenAddress,              // ERC-20 代币地址
    ...
    if (error.message?.includes("in-flight transaction limit")) {
      ...
      console.log(
        "2. 在 Etherscan 上检查你的地址是否有待处理交易: https://sepolia.etherscan.io/address/" +
          signer.address,
      );
    }
