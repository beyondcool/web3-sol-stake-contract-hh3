...
async function main() {
  ...
  try {
    ...
    const cancelGasPrice = (currentGasPrice * 300n) / 100n; // 3 倍当前价格
    const finalGasPrice =
      cancelGasPrice > minGasPrice ? cancelGasPrice : minGasPrice;

    console.log(
      "取消交易使用的 gas price:",
      ethers.formatUnits(finalGasPrice, "gwei"),
      "Gwei",
    );
    console.log("\n发送取消交易...");
    ...
    console.log(
      "在 Etherscan 上查看: https://sepolia.etherscan.io/tx/" + cancelTx.hash,
    );
    console.log("\n等待交易确认...");
    ...
    // 再次检查 nonce
    const newCurrentNonce = await ethers.provider.getTransactionCount(
      signer.address,
      "latest",
    );
    const newPendingNonce = await ethers.provider.getTransactionCount(
      signer.address,
      "pending",
    );
    ...
    if (newPendingNonce === newCurrentNonce) {
      ...
      console.log(
        "⚠️  还有",
        newPendingNonce - newCurrentNonce,
        "个交易待处理",
      );
    ...
    console.log(
      "4. 使用 Etherscan 的取消功能: https://sepolia.etherscan.io/address/" +
        signer.address,
    );
